#include "ADComputePlasticHeatingInitial.h"
#include "RankTwoTensor.h"

registerMooseObject("mistApp", ADComputePlasticHeatingInitial);

InputParameters
ADComputePlasticHeatingInitial::validParams()
{
    ...
    params.addRequiredCoupledVar("dirac_switch_shock", "dirac_switch_shock");
...
ADComputePlasticHeatingInitial::ADComputePlasticHeatingInitial(const InputParameters & parameters)
  : Material(parameters),
...
void
ADComputePlasticHeatingInitial::computeQpProperties()
{
    ...
    if(_dirac_switch_shock[_qp] > _thr_activation){
