...
#include "ComputeIsoPlasticPFFractureStressDegr.h"

registerMooseObject("TensorMechanicsApp", ComputeIsoPlasticPFFractureStressDegr);

InputParameters
ComputeIsoPlasticPFFractureStressDegr::validParams()
{
...
ComputeIsoPlasticPFFractureStressDegr::ComputeIsoPlasticPFFractureStressDegr(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _deltaT(declareProperty<Real>("deltaT")),
    _pdeg(declareProperty<Real>("pdeg")),
    _sigeqv(declareProperty<Real>("sigeqv")),
    _F_posvol(declareProperty<Real>("F_posvol")),
    _F_posdev(declareProperty<Real>("F_posdev")),
    _F_neg(declareProperty<Real>("F_neg")),
    ...
    _stress_deg(declareProperty<RankTwoTensor>(_base_name +"stress_deg")),
    _stress_p(declareProperty<RankTwoTensor>(_base_name +"stress_p")),
    _stress0neg(declareProperty<RankTwoTensor>(_base_name +"stress0neg")),
    _stress0pos(declareProperty<RankTwoTensor>(_base_name +"stress0pos"))
...
void
ComputeIsoPlasticPFFractureStressDegr::initQpStatefulProperties()
{
...
void
ComputeIsoPlasticPFFractureStressDegr::computeQpStress()
{
  ...
  delta = _elastic_strain[_qp].trace();
  eta = - delta;
  ...
  Real  jacob;
  jacob = 1.0 + _elastic_strain[_qp].trace();

  //if (jacob > 1.0) { // expansion, use hookes law
    peos = - K0 * eta  - _Gamma * _density[_qp] * _specific_heat[_qp] * (temperature - _ref_temperature);
  //}
  ...
  _deltaT[_qp]= temperature - _ref_temperature;
  ...
  _sigeqv[_qp]=getSigEqv(stress);// check vonmises
  ...
  peos = - K0 * eta  - _Gamma * _density[_qp] * _specific_heat[_qp] * (temperature - _ref_temperature);
  //peos = stress.trace()/3; // should not change afterplasticity?
  stress_eos = peos * I2;
  _stress_eos_elastic[_qp] = stress_eos;
  stress_cpl = _elasticity_tensor[_qp] * (_elastic_strain[_qp]) - K0 * delta * I2;// updated elastic strain here
  _stress_cpl_elastic[_qp] = stress_cpl;
  stress = stress_eos + stress_cpl;
///////////////////////////////////////////////////////////////
// up till this point, we have the updated stress, peos, elastic_strain, plastic strain


  stress_eos = peos * I2;// spherical stress
  //_stress_eos_plastic[_qp] = stress_eos;
  stress_cpl = stress - stress_eos; // deviatoric stress
  ...
  RankTwoTensor strain0vol, strain0dev;
    ...
    // was using mechanical strain, now using elastic strain
    strain0dev = _elastic_strain[_qp].deviatoric();
    strain0vol = _elastic_strain[_qp] - strain0dev;
    strain0tr = _elastic_strain[_qp].trace();
    strain0tr_neg = std::min(strain0tr, 0.0);
    strain0tr_pos = strain0tr - strain0tr_neg;


    stress0pos = k * strain0tr_pos * I2 + mu * strain0dev;//elastic only
    stress0neg =stress - stress0pos; // why is this non-zero in hotspot, when F_neg=0

    // Energy with positive principal strains
  RankTwoTensor strain0dev2 = strain0dev * strain0dev;
  // one of the tr_pos tr_neg is 0
  F_pos = 0.5 * k * strain0tr_pos * strain0tr_pos + mu * strain0dev2.trace();
  F_neg = 0.5 * k * strain0tr_neg * strain0tr_neg;
  _F_posvol[_qp]=0.5 * k * strain0tr_pos * strain0tr_pos;
  _F_posdev[_qp]=mu * strain0dev2.trace();
  _F_neg[_qp]=0.5 * k * strain0tr_neg * strain0tr_neg;
  _pdeg[_qp]=_pressure[_qp];
  _stress[_qp] = stress0pos * _D[_qp] - _pressure[_qp] * I2 * _I[_qp] + stress0neg;

_stress_deg[_qp]=stress0pos * _D[_qp];
_stress_p[_qp]=- _pressure[_qp] * I2 * _I[_qp] ;
_stress0pos[_qp]=stress0pos;
_stress0neg[_qp]=stress0neg;
  ...
  if (strain0tr < 0)    Jacobian_neg = k * I2I2;
...
Real
ComputeIsoPlasticPFFractureStressDegr::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
ComputeIsoPlasticPFFractureStressDegr::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
ComputeIsoPlasticPFFractureStressDegr::dyieldFunction_dinternal(const Real equivalent_plastic_strain)
{
  return -getdYieldStressdPlasticStrain(equivalent_plastic_strain);
}

RankTwoTensor
ComputeIsoPlasticPFFractureStressDegr::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
ComputeIsoPlasticPFFractureStressDegr::internalPotential()
{
  return -1;
}

Real
ComputeIsoPlasticPFFractureStressDegr::getSigEqv(const RankTwoTensor & stress)
{
...
void
ComputeIsoPlasticPFFractureStressDegr::getJac(const RankTwoTensor & sig,
...
Real
ComputeIsoPlasticPFFractureStressDegr::getYieldStress(const Real eqpe)
{
  unsigned nsize;
...
Real
ComputeIsoPlasticPFFractureStressDegr::getdYieldStressdPlasticStrain(const Real eqpe)
{
  unsigned nsize;
  nsize = _yield_stress_vector.size();
