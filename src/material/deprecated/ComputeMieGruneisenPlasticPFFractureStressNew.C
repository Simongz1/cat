...
#include "ComputeMieGruneisenPlasticPFFractureStressNew.h"

registerMooseObject("TensorMechanicsApp", ComputeMieGruneisenPlasticPFFractureStressNew);

InputParameters
ComputeMieGruneisenPlasticPFFractureStressNew::validParams()
{
...
ComputeMieGruneisenPlasticPFFractureStressNew::ComputeMieGruneisenPlasticPFFractureStressNew(const InputParameters & parameters)
...
void
ComputeMieGruneisenPlasticPFFractureStressNew::initQpStatefulProperties()
{
...
void
ComputeMieGruneisenPlasticPFFractureStressNew::computeQpStress()
{
  ...
  peos = - K0 * eta * (1.0 - (_Gamma * eta / 2.0)) / std::pow((1.0 - _s * eta), 2.0) - _Gamma * _density[_qp] * _specific_heat[_qp] * (temperature - _ref_temperature) * eta;
  
  if (_c[_qp] > _thr_gas) {
    peos -= _c[_qp] * _k_alpha * _Gamma * _density[_qp] * _specific_heat[_qp] * (temperature - _ref_temperature) * eta; //gas expansion pressure term
  }
...
Real
ComputeMieGruneisenPlasticPFFractureStressNew::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
ComputeMieGruneisenPlasticPFFractureStressNew::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
ComputeMieGruneisenPlasticPFFractureStressNew::dyieldFunction_dinternal(const Real eqvpstrain, const Real eqv_plastic_strain_rate, const Real Temperature)
{
  return -getdYieldStressdPlasticStrain(eqvpstrain, eqv_plastic_strain_rate, Temperature);
}

RankTwoTensor
ComputeMieGruneisenPlasticPFFractureStressNew::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
ComputeMieGruneisenPlasticPFFractureStressNew::internalPotential()
{
  return -1;
}

Real
ComputeMieGruneisenPlasticPFFractureStressNew::getSigEqv(const RankTwoTensor & stress)
{
...
void
ComputeMieGruneisenPlasticPFFractureStressNew::getJac(const RankTwoTensor & sig,
...
Real
ComputeMieGruneisenPlasticPFFractureStressNew::getYieldStress(const Real eqvpstrain, const Real eqv_plastic_strain_rate, const Real Temperature)
{
...
Real
ComputeMieGruneisenPlasticPFFractureStressNew::getdYieldStressdPlasticStrain(const Real eqvpstrain, const Real eqv_plastic_strain_rate, const Real Temperature)
{
  Real sigma_Y_1, sigma_Y_2, sigma_Y_3, sigma_Y, theta, T_melt;
  Real dy_dplastic;
  // _eqv_plastic_strain_rate[_qp] = (_eqv_plastic_strain[_qp] - _eqv_plastic_strain_old[_qp]) / _dt;
  ...
  Real dy_dplastic_1, dy_dplastic_2;
