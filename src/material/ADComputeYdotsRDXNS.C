...
registerMooseObject("mlApp", ADComputeYdotsRDXNS);
...
ADComputeYdotsRDXNS::computeQpProperties()
{
    ...
    condition_chemistry = (_dirac_switch_react[_qp] > ADReal(1.0) ? true : false);
    ...
    if(_dirac_switch_react[_qp] > _time_react[_qp]){ //turn on after reaction heat has gone by
        //compute the rates of reaction of all reactions
        _r1[_qp] = _Z1 * MetaPhysicL::exp(- _E1 / (R_RDX * _T[_qp]));
        _r2[_qp] = _Z2 * MetaPhysicL::exp(- _E2 / (R_RDX * _T[_qp]));
    }

    _Q1[_qp] = (_a1) + (_b1 * std::max(ADReal(0.0), _T[_qp] - _T_trans));
    _Q2[_qp] = (_a2) + (_b2 * std::max(ADReal(0.0), _T[_qp] - _T_trans));
