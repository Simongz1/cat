...
#include "ComputeIsoPlasticPFFractureStressDegrEspec.h"

registerMooseObject("TensorMechanicsApp", ComputeIsoPlasticPFFractureStressDegrEspec);

InputParameters
ComputeIsoPlasticPFFractureStressDegrEspec::validParams()
{
...
ComputeIsoPlasticPFFractureStressDegrEspec::ComputeIsoPlasticPFFractureStressDegrEspec(const InputParameters & parameters)
...
void
ComputeIsoPlasticPFFractureStressDegrEspec::initQpStatefulProperties()
{
...
void
ComputeIsoPlasticPFFractureStressDegrEspec::computeQpStress()
{
  ...
  stress_eos = peos * I2;
  _stress_eos_elastic[_qp] = stress_eos;
  stress_cpl = _elasticity_tensor[_qp] * (_elastic_strain[_qp]) - K0 * delta * I2;
  _stress_cpl_elastic[_qp] = stress_cpl;
  stress = stress_eos + stress_cpl;
///////////////////////////////////////////////////////////////
// up till this point, we have the updated stress, peos, elastic_strain, plastic strain
  ...
  RankFourTensor Ppos =
      _elastic_strain[_qp].positiveProjectionEigenDecomposition(eigval, eigvec);
  ...
  for (const auto i : make_range(Moose::dim))
  {
    ...
    //stress0neg += etens[i] * (lambda * etrneg + 2.0 * mu * eneg[i]); changed, otherwise stresses always zero 
  }
...
Real
ComputeIsoPlasticPFFractureStressDegrEspec::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
ComputeIsoPlasticPFFractureStressDegrEspec::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
ComputeIsoPlasticPFFractureStressDegrEspec::dyieldFunction_dinternal(const Real equivalent_plastic_strain)
{
  return -getdYieldStressdPlasticStrain(equivalent_plastic_strain);
}

RankTwoTensor
ComputeIsoPlasticPFFractureStressDegrEspec::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
ComputeIsoPlasticPFFractureStressDegrEspec::internalPotential()
{
  return -1;
}

Real
ComputeIsoPlasticPFFractureStressDegrEspec::getSigEqv(const RankTwoTensor & stress)
{
...
void
ComputeIsoPlasticPFFractureStressDegrEspec::getJac(const RankTwoTensor & sig,
...
Real
ComputeIsoPlasticPFFractureStressDegrEspec::getYieldStress(const Real eqpe)
{
  unsigned nsize;
...
Real
ComputeIsoPlasticPFFractureStressDegrEspec::getdYieldStressdPlasticStrain(const Real eqpe)
{
  unsigned nsize;
  nsize = _yield_stress_vector.size();
