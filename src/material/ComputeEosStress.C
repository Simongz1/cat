...
#include "ComputeEosStress.h"

registerMooseObject("TensorMechanicsApp", ComputeEosStress);

InputParameters
ComputeEosStress::validParams()
{
...
ComputeEosStress::ComputeEosStress(const InputParameters & parameters)
...
void
ComputeEosStress::initQpStatefulProperties()
{
  ComputeStressBase::initQpStatefulProperties();
}

void
ComputeEosStress::computeQpStress()
{
  ...
  _peos1[_qp]= - K0 * eta * (1.0 - (_Gamma * eta / 2.0)) / std::pow((1.0 - _s * eta), 2.0); //non linear mechanical contribution
  _peos2[_qp]= _Gamma * _density[_qp] * _specific_heat[_qp] * (temperature - _ref_temperature); //thermal dependent contribution (alpha*deltaT)
  stress_eos = peos * I2;
