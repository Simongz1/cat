...
registerMooseObject("mistApp", ADPBXStress);
...
ADPBXStress::initialSetup()
{
  ...
  if (!hasGuaranteedMaterialProperty(_elasticity_tensor_name, Guarantee::ISOTROPIC))
    mooseWarning("ADPBXStress requires an isotropic elasticity tensor");
}
...
void
ADPBXStress::computeQpPK1Stress()
{
...
Real
ADPBXStress::computeReferenceResidual(const Real & effective_trial_stress,
                                                              const Real & scalar)
{
...
Real
ADPBXStress::computeResidual(const Real & effective_trial_stress,
                                                     const Real & scalar)
{
...
Real
ADPBXStress::computeDerivative(const Real & /*effective_trial_stress*/,
                                                       const Real & scalar)
{
  // Update the flow stress
  _ep[_qp] = _ep_old[_qp] + scalar;
...
void
ADPBXStress::preStep(const Real & scalar, const Real & R, const Real & J)
{
  if (!_fe_problem.currentlyComputingJacobian())
    return;
