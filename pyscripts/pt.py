...
nslices = num_slices
locs = np.linspace(0, size, nslices + 1) #this creates a fixed number of locations
...
match dimension:
    case '3D':
        for loc in locs:
            ...
            #append fixed slicer to the slicer collection, this keeps them fixed in time
...
idx = np.unique(np.round(np.linspace(min_time, max_time, dt)).astype(int))
times = ts[idx]
...
for it, t in enumerate(times): 
    for ix, sl in enumerate(slicers):
        match dimension:
            case '2D':
            ...
            case '3D':
                sl.UpdatePipeline(time=t)
        
        #compute statistics
        data_raw = dsa.WrapDataObject(servermanager.Fetch(sl))
        if (vartype == 'POINT'):
...
norm = mcolors.Normalize(vmin=float(ps.min()), vmax=float(ps.max()))
sm = cm.ScalarMappable(norm=norm, cmap=cmap)

for ix, p in enumerate(ps):
    color = cmap(norm(float(p)))
    
    ax.plot(times, np.abs(avg[:, ix]), color='k', linewidth = 2)
    if (include_stats == 'YES'):
        ax.plot(times, np.abs(Q1[:, ix]), color='k', linewidth = 0.5, alpha=0.35)
        ax.plot(times, np.abs(Q3[:, ix]), color='k', linewidth = 0.5, alpha=0.35)
        ax.plot(times, np.abs(mins[:, ix]), color='k', linewidth = 0.5, alpha=0.35)
        ax.plot(times, np.abs(maxs[:, ix]), color='k', linewidth = 0.5, alpha=0.35)
        
        ax.fill_between(times, np.abs(Q1[:, ix]), np.abs(Q3[:, ix]), color = color, alpha=0.25, linewidth=0)
        ax.fill_between(times, np.abs(mins[:, ix]), np.abs(maxs[:, ix]), color = color, alpha=0.1, linewidth=0)
...
cbar.set_label('Distance ($\mu m$)', fontsize=20)
plt.tight_layout()
plt.savefig(f'file{filename.replace("/", "_")}_{field}_over_t_{num_slices}dx_{num_times}dt.jpeg', dpi=1000, bbox_inches='tight')
plt.show()
