#include "ADPBXTrueStress.h"

registerMooseObject("mistApp", ADPBXTrueStress);

InputParameters
ADPBXTrueStress::validParams()
{
...
void
ADPBXTrueStress::initialSetup()
{
  _flow_stress_material = &getMaterial("flow_stress_material");

  // Enforce isotropic elastic tensor
  if (!hasGuaranteedMaterialProperty(_elasticity_tensor_name, Guarantee::ISOTROPIC))
    mooseWarning("ADPBXTrueStress requires an isotropic elasticity tensor");
}

void
ADPBXTrueStress::initQpStatefulProperties()
{
...
void
ADPBXTrueStress::computeQpPK1Stress()
{
  ...
  //
  RankTwoTensor s = _mixture_shear * _be[_qp].deviatoric();
  ...
  if (computeResidual(s_eff, 0) > 0)
  {
    ...
    if (_fe_problem.currentlyComputingJacobian())
    {
      ...
      if (MooseUtils::absoluteFuzzyEqual(s.norm(), 0))
        ...
        _d_n_d_be = _mixture_shear / std::sqrt(6) / s.norm() *
  ...
  if (_fe_problem.currentlyComputingJacobian())
  {
    RankFourTensor d_tau_d_F = _mixture_bulk * detJ * detJ * I.times<i, j, k, l>(Fit) +
                               _mixture_shear * (_d_be_d_F - I.times<i, j, k, l>(I) * _d_be_d_F / 3);
...
Real
ADPBXTrueStress::computeReferenceResidual(const Real & effective_trial_stress,
                                                              const Real & scalar)
{
  
  return effective_trial_stress - _mixture_shear * scalar * _be[_qp].trace();
}

Real
ADPBXTrueStress::computeResidual(const Real & effective_trial_stress,
                                                     const Real & scalar)
{
...
Real
ADPBXTrueStress::computeDerivative(const Real & /*effective_trial_stress*/,
                                                       const Real & scalar)
{
  // Update the flow stress
  _ep[_qp] = _ep_old[_qp] + scalar;
...
void
ADPBXTrueStress::preStep(const Real & scalar, const Real & R, const Real & J)
{
  if (!_fe_problem.currentlyComputingJacobian())
    return;
